...
# 另外一种实现
当进程试图在内核锁文件等级失败的时候,不在阻塞队列里睡大觉,而是直接返回加锁失败.这是一种非阻塞机制.  
对于阻塞等待的机制,不太好实现超时机制,即使有中断,但依然很难实现.  
使用费阻塞机制 + time.sleep()就非常的容易实现.同时在win平台中就是通过轮询+sleep来实现超时的.实现了多平台的统一.  
现在通常的进程锁都使用非阻塞 + 超时 + 轮询 机制实现.  
