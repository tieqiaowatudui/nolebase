...
## 通配符目标
我们可以在目标名称中使用通配符,这样能够帮助我们将整个目录中的多个目录生成目标.
例如:
```c
build/%: src/%.c | build
	$(CC) $(CFLAGS) $< -o $@
...
这里可以在build/% 批量定义目标.将生成的结果.
其中:
`$<`:表示第一个依赖.名称为 `src/main.c`
`$@`:指向目标.名称为 `build/main`
...
# 总结
make就是一个 围绕 目标,依赖,指令 这个单位展开的构建工具.make管理了整个定义的所有目标的最新情况,精确的定义了构建过程,减少无用的构建.